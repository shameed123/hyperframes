(function () {
  ...
  /**
   * Detect whether a <style> element was injected by a browser extension
   * (e.g. ad blockers like uBlock Origin / AdBlock Plus) rather than being
   * part of the original HTML.
   *
   * Used by both slide-inner (iframe context, Rollup bundle) and
   * html_editor (parent page, Nuxt/Vite bundle).
   */
  function isExtensionInjectedStyle(styleEl) {
    if (styleEl.id) return false
    const content = styleEl.textContent || '';
    if (
      content.length > 20000 &&
      /display:\s*none\s*!important/.test(content) &&
      /[.#]ad[-_s]|\.advert|\.sponsor/i.test(content)
    )
      return true
    return false
  }
  ...
  function slideUpdateHtml() {
    ...
    // Remove browser extension injected styles (ad blockers, Dark Reader, etc.)
    doc.querySelectorAll('head style').forEach(style => {
      if (isExtensionInjectedStyle(style)) {
        style.remove();
      }
    });
